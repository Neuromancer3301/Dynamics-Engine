 ...
 * a single-slot, lock-free mailbox holding the most recent state snapshot.
 *
 ...
 * one, never a mixture. Combined with {@code S} being deeply immutable (see
 * {@code physics.SimState} for the pendulum's), that is all the
 * synchronisation required: there is no shared mutable data left to protect.
 * No {@code synchronized}, no lock contention, no chance of deadlock.
 *
 ...
 *
 * <p><b>Round 2 §2:</b> genericized over {@code S} rather than hardcoded to
 * {@code physics.SimState} — the mailbox mechanics never inspected the
 * snapshot's contents, so this costs nothing and lets a future simulation
 * type (n-body, smoke) reuse it with its own state type.
 *
 * @param <S> the state snapshot type being published
 */
public final class StateBuffer<S> {
    ...
    private final AtomicReference<S> slot = new AtomicReference<>();
